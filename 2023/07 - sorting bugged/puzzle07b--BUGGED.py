...
def get_high_card(hand):
    for value in ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']:
        if value in hand:
            return value
...
for hand, bid in hands:
    ...
    if counted.most_common(1)[0][0] == 'J':
        if len(counted) == 1:
            max_repeat = 5  # only jokers
        else:
            max_repeat = counted.most_common(2)[1][1]
    else:
    ...
    try:
        ...
        next_repeat = 0
    if (max_repeat == 5 or
            (max_repeat == 4 and jokers == 1) or
            (max_repeat == 3 and jokers == 2) or
            (max_repeat == 2 and jokers == 3) or
            (max_repeat == 1 and jokers == 4)):
        # print("5 of a kind")
        hands_ranked[0].append((hand, bid))
    elif (max_repeat == 4 or
          (max_repeat == 3 and jokers == 1) or
          (max_repeat == 2 and jokers == 2) or
          (max_repeat == 1 and jokers == 3)):
        # print("4 of a kind")
        hands_ranked[1].append((hand, bid))
    elif ((max_repeat == 3 and next_repeat == 2) or
          (max_repeat == 2 and next_repeat == 2 and jokers == 1)):
        # print("full house")
        hands_ranked[2].append((hand, bid))
    elif (max_repeat == 3 or
          (max_repeat == 2 and jokers == 1) or
          (max_repeat == 1 and jokers == 2)):
        # print("3 of a kind")
        hands_ranked[3].append((hand, bid))
    elif (max_repeat == 2 and next_repeat == 2):
        # print("two pair")
        hands_ranked[4].append((hand, bid))
    elif (max_repeat == 2 or
          (max_repeat == 1 and jokers == 1)):
        # print("one pair")
...
for rank in hands_ranked:
    SORT_ORDER = {'A': 0, 'K': 1, 'Q': 2, 'T': 4, '9': 5, '8': 6, '7': 7, '6': 8, '5': 9, '4': 10, '3': 11, '2': 12, 'J': 13}
